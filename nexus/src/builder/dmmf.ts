...
import type { DMMFClass } from '@prisma/client/runtime/dmmf'
...
export const getDmmf = (packageOrClient: any = '@prisma/client'): DMMFClass => {
  if (typeof packageOrClient === 'string') {
    return new RuntimeDMMFClass(getDmmfDocument(packageOrClient))
  }

  if (packageOrClient.dmmf) {
    return new RuntimeDMMFClass(packageOrClient.dmmf)
  }
